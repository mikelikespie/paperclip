module Paperclip
  module Storage
    module Async
      ...
      def save
        return if storage_proxy.processing?(name, instance)
        super
      end
      ...
      def to_file(style = default_style)
        ...
        return super unless (proxy = storage_proxy.new(name, instance)).processing?
      ...
      def url(style = default_style, include_updated_timestamp = true)
        storage_proxy.processing?(:"#{@name}", @instance) ? interpolate(@default_url, style) : super
      end
    end
  end
end
